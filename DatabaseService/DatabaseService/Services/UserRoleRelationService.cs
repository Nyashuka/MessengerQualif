...
namespace DatabaseService.Services
{
    public class UserRoleRelationService : IUserRoleRelationService
    {
        ...
        public async Task<ServiceResponse<bool>> AsignDefaultRole(int chatId, int userId)
        {
            var defaultRole = await _databaseContext.DefaultRoles.FirstOrDefaultAsync(x => x.ChatId == chatId);

            if (_databaseContext.UserRoleRelations.Any(x => x.RoleId == defaultRole.RoleId && x.UserId == userId))
            {
                return new ServiceResponse<bool>() { Data = false, Success = false, Message = "User already have this role" };
            }
        ...
        public async Task<ServiceResponse<bool>> RemoveRole(int roleId, int userId)
        {
            ...
            if (isDefaultRole)
            {
                return new ServiceResponse<bool>()
                {
                    Data = false,
                    Success = false,
                    Message = "Default role can't be unassigned"
                };
            }
